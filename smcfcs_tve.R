...
smcfcs.tve<- function(originaldata,id,smformula.timefixed,method,tvar=NULL,tvarfunc=NULL,spline=NULL,knots=NULL,nknots=NULL,predictorMatrix=NULL,m=5,numit=10,rjlimit=1000,noisy=FALSE,quick=T,quick.cuts=NULL,
  ...
  d <- originaldata[,dCol]
  explanCol<-(1:dim(originaldata)[2])[colnames(originaldata) %in% unlist(strsplit(toString(as.formula(smformula)[[3]]),", "))] #RUTH
  ...
  if(!is.null(tvarfunc)) print(paste("The TVE function is tvarfunc."))
  if(!is.null(spline) & !is.null(nknots)) print(paste("The TVE function is a restricted cubic spline with ",nknots," knots."))
  if(!is.null(spline) & !is.null(knots)) print(paste("The TVE function is a restricted cubic spline with knots",nknots))
